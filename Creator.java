...
public class Creator {
        ...
        while (true) {
            ...
            switch (response) {
                ...
                case 3:
                    ...
                    createdAnimals = deleteFromAnimals(createdAnimals, deleteID);
                    System.out.println("-------------------------------------------");
                    break;
    ...
    public static Animal[] addToAnimals(Animal[] animals, Animal newAnimal)
    {
        ...
        //create the new pointer to object so id can be another
        return newAnimals;
    }

    public static Animal[] deleteFromAnimals(Animal[] animals, int deleteID)
    {
    ...
    public void writeToFile()
    {
        try
        {
            FileWriter writer = new FileWriter("creator_info.txt");
            writer.write(creatorPass + "\n");

            writer.write(createdAnimals.length + "\n");
            for (Animal animal: createdAnimals)
            {
                writer.write(animal.prepToFile() + "\n");
            }
            writer.close();
            System.out.println("Information was saved in the file(creator mode).");
        }
        catch(IOException e)
        {
    ...
    public void readFromFile()
    {
        try
        {
            ...
            BufferedReader reader = new BufferedReader(new FileReader("creator_info.txt"));
            this.creatorPass = reader.readLine();

            int amount = Integer.parseInt(reader.readLine());
            createdAnimals = new Animal[amount];
            for(int i = 0; i < amount; i++)
            {
                String info = reader.readLine();
                createdAnimals[i] = Animal.constructAnimal(info);
                findAFood(createdAnimals[i].type, createdAnimals[i].name);
                createdAnimals[i].id = i + 1;
            }
            reader.close();
            System.out.println("Information was read from the file(creator).");
        }
        catch(IOException e)
        {
